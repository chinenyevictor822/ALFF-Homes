...
export default function Navigation() {
  ...
  const handleLinkClick = (e: React.MouseEvent<HTMLAnchorElement>, link: typeof navLinks[0]) => {
    ...
    if (location.pathname !== "/") {
      navigate("/");
      window.setTimeout(() => document.querySelector(link.to)?.scrollIntoView({ behavior: "smooth" }), 50);
    } else {
        ...
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
            ...
            {navLinks.map((link) => (
              link.isExternal ? (
                <Link
                  ...
                  className={`text-xs uppercase tracking-[0.2em] font-medium transition-colors duration-300 relative py-1 after:absolute after:bottom-0 after:left-0 after:w-full after:h-[1px] after:bg-brand-bronze after:scale-x-0 hover:after:scale-x-100 after:transition-transform after:duration-500 after:origin-left focus:outline-hidden focus:ring-2 focus:ring-brand-bronze ${
          ...
          <div className="md:hidden">
            <button
              ...
              className="text-brand-charcoal hover:text-brand-bronze p-1.5 focus:outline-hidden focus:ring-2 focus:ring-brand-bronze"
      ...
      >
        <div className="flex flex-col space-y-8 mt-4">
          ...
          {navLinks.map((link, idx) => (
            link.isExternal ? (
              <Link key={link.label} to={link.to} onClick={() => setIsMobileMenuOpen(false)} className={`font-serif text-3xl transition-colors duration-300 block focus:outline-hidden focus:ring-2 focus:ring-brand-bronze ${location.pathname === link.to ? "text-brand-bronze" : "text-brand-charcoal hover:text-brand-bronze"}`} style={{ transitionDelay: `${idx * 75}ms` }}>
                {link.label}
              </Link>
            ) : (
              <a key={link.label} href={link.to} onClick={(e) => handleLinkClick(e, link)} className="font-serif text-3xl text-brand-charcoal hover:text-brand-bronze transition-colors duration-300 block focus:outline-hidden focus:ring-2 focus:ring-brand-bronze" style={{ transitionDelay: `${idx * 75}ms` }}>
                {link.label}
              </a>
            )
          ))}
        ...
        <div className="flex flex-col space-y-6">
          <Link to="/properties" onClick={() => setIsMobileMenuOpen(false)} className="flex items-center justify-between border-b border-brand-charcoal py-4 text-xs uppercase tracking-[0.2em] text-brand-charcoal font-medium hover:text-brand-bronze transition-colors focus:outline-hidden focus:ring-2 focus:ring-brand-bronze">
